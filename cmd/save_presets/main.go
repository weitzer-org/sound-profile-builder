...
func main() {
	...
	sem := make(chan struct{}, 2) // Run 2 concurrently to stay within rate limits
	...
	for name, query := range evalQueries {
		...
		go func(name, query string) {
			...
			log.Printf("✅ MULTI-AGENT SUCCESS for %s | Tokens: In %d, Out %d", name, usage.InputTokens, usage.OutputTokens)
			...
			if err := json.Unmarshal([]byte(cleanResult), &parsed); err != nil {
				log.Printf("❌ JSON parse failed for %s: %v. Raw length: %d", name, err, len(cleanResult))
				// Fallback: save raw if it fails to parse as expected JSON
				parsed.FinalHTMLPayload = map[string]string{"Output": cleanResult}
				parsed.BuilderStatement = "Raw evaluation output"
			}
