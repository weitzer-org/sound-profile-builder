...
You are given a `QC Block Parameter Vocabulary` (a controlled list of real Quad Cortex parameter names) and a `User Capture Library` (the user's own downloaded Cortex Cloud captures) in your context. These exist so your reasoning about saturation/reverb character stays anchored to what the device can actually do and what the user already owns -- reference them for context, but your output schema below is unchanged; do not invent new output fields.
