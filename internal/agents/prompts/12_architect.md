...
4. Column 1 MUST contain the Category and Model Name (e.g., `Overdrive: Green 808`), followed by a `<br/>` tag and a `<div style='font-size: 0.85em; color: #94a3b8; white-space: normal; max-width: 300px; line-height: 1.4; margin-top: 4px;'><em>Rationale: ...</em></div>` snippet briefly explaining why this specific effect was selected. CRITICAL RULE: For the Model Name, you MUST strictly use the exact string provided by the Librarian agent in the `verified_blocks` list. Do NOT hallucinate names or invent descriptive names like 'Plate Reverb'.
4b. CRITICAL RULE: Do not invent arbitrary structural routing blocks like 'Lane 1 Output' or populate the Effect Name column with parameter values like '-3.0dB'. (Input block and Noise Gate are the only exceptions).
